...
export const DOC_SLOTS = ['id_front', 'id_back', 'passport', 'selfie', 'licence_front', 'licence_back', 'taxi_licence', 'vehicle_reg', 'insurance', ...VEHICLE_PHOTO_SLOTS];
...
export async function requestChanges(applicationId: string, adminId: string, reason: string): Promise<Result> {
  if (!reason || reason.trim().length < 3) return err(422, 'REASON_REQUIRED', 'Describe the changes required.');
  return runAdmin(applicationId, adminId, ['SUBMITTED', 'IN_REVIEW'], 'CHANGES_REQUESTED', 'CHANGES_REQUESTED', reason, true);
}
export async function rejectApplication(applicationId: string, adminId: string, reason: string): Promise<Result> {
  if (!reason || reason.trim().length < 3) return err(422, 'REASON_REQUIRED', 'A rejection reason is required.');
  return runAdmin(applicationId, adminId, ['SUBMITTED', 'IN_REVIEW'], 'REJECTED', 'REJECTED', reason);
}

async function runAdmin(applicationId: string, adminId: string, from: string[], to: string, evt: string, reason?: string, bumpRevision = false): Promise<Result> {
...
export async function setDocumentDecision(applicationId: string, docId: string, decision: 'ACCEPTED' | 'CHANGES', note: string | undefined, adminId: string): Promise<Result> {
  ...
  await prisma.applicationDocument.update({ where: { id: docId }, data: { decision, decisionNote: note ?? null } });
