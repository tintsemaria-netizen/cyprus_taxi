> STATUS (2026-09-13): M0 + M1 + M2 + M3 DONE; M4 substantially DONE (dynamic pricing, weather adapter, Places (New) autocomplete w/ fallback, isolated load test). Remaining: REGULATED_FIXED airport fares; live activation of Places API (New) + an authorized weather provider (both are external-enablement blockers, not code gaps). See per-milestone notes below.
...
> - DONE M3: scheduled-ride automation (worker promotes REQUESTED→SEARCHING within a 15-min lead, deadline runs to pickup time); arrival rules (EN_ROUTE→ARRIVED needs fresh GPS within 150m of pickup, airports exempt) + persisted WaitingSession with 3-min free grace (pre-pickup paid rate 0 in regulated mode); code-gated start (4-digit start code issued at assignment, shown to passenger, driver enters it ARRIVED→IN_PROGRESS); immutable Fare receipt written once at COMPLETED with payment status separate (CASH_TO_DRIVER/PENDING, never auto-collected); pre-pickup driver cancel → automatic rematch preserving the fare snapshot; prolonged pre-pickup GPS loss (90s) → worker rematch, never for IN_PROGRESS. New driver endpoints arrive/start/complete/cancel; driver UI (arrival, start-code entry, waiting timer, release) and passenger UI (start code, waiting timer, trip receipt, scheduled "reservation received"). Migration 20260913160027_m3_waiting_fare_scheduling. vitest 41/41.
> - DONE (M4, mostly): (1) UPFRONT_DYNAMIC synthetic pricing — bounded [1.0,1.5] deterministic demand/supply multiplier with min-sample threshold, hysteresis, weather-into-demand (non-overlapping, capped), zero-supply≠infinite; applied only to eligible base (initial+distance); config-gated (PRICING_MODE, default REGULATED_METER_ESTIMATE so real charges stay regulated). src/lib/pricing-dynamic.ts + src/server/dispatch/market.ts (supply/demand snapshot). (2) Weather adapter src/server/weather.ts — timeout/freshness/NEUTRAL fallback; providers none(default)/fixture/open-meteo (open-meteo left OFF pending commercial authorization). (3) Places API (New) autocomplete + details (src/server/google.ts googleAutocomplete/googlePlaceDetails, /api/v1/places/{search,details}, session token) with graceful fallback to forward geocoding when Places is not enabled; client PlacesInput now has session lifecycle, keyboard nav, and prediction→details resolution (keeps debounce/cancel/stale-guard). (4) Isolated load test scripts/load-test.ts — 100 concurrent bookings against a 1000-driver fixture (Google stubbed): 100/100 assigned, ZERO invariant violations, p50/p95 assignment latency ~4.9s/6.6s at 1000 drivers (~1.3s/1.8s at 120). vitest 52/52.
> - PENDING (M4 remainder): REGULATED_FIXED airport/zone fixed fares; ENABLE Places API (New) on the Google project to light up real autocomplete (code falls back to geocoding until then); authorize a commercial weather provider to activate real weather (neutral until then). These are external-enablement blockers, not code gaps.
