> STATUS (2026-09-14): P0 DONE; P1 DONE (incl. scheduled-time pricing); P2 DONE (chat cursor pagination). All deployed. vitest 69/69. Live server-side smoke of the full journey PASSED on prod (see HANDOFF). Remaining (documented next actions): ETA source-exposure in the passenger UI (server already distinguishes traffic vs approx), notification outbox (delivery is best-effort), and a real on-phone browser journey (device GPS + push).
>
> Completion matrix:
