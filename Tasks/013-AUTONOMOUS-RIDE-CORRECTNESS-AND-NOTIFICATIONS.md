> STATUS (2026-09-14): P0 DONE; P1 DONE; P2 DONE. Notification reliability + chat authorization COMPLETED (release 31d5781): chat send now validates chat-open + active-assignment ownership atomically under the booking row lock; notifications go through a durable NotificationOutbox (enqueue-in-domain-write, worker drain with retries/backoff + dedupe, stale offers dropped). vitest 78/78. Remaining (minor/optional): ETA source-exposure in the passenger UI (server already distinguishes traffic vs approx); a real on-phone browser journey (device GPS + push).
>
> Completion matrix:
...
> - P1 quote timing — DONE. quoteCreatedAt/expiry are separate from pricingAt: a scheduled ride is priced (day/night/holiday tariff) AND routed (traffic-aware future departureTime) for its journey time, while expiry stays relative to issuance; the quote hash binds the scheduled time + luggage + quoteId (a changed schedule/luggage/quote invalidates it); Quote persists pricingAt + routeSource. Dispatcher-confirmed wording removed; upfront vs regulated settlement fixed in completion. (Verified: tests/pricing.db.test.ts — day-issued night-journey quote uses night tariff; expiry stays 120s from issuance; changed schedule → QUOTE_MISMATCH.)
...
> - P2 chat continuity — DONE. Stable (createdAt,id) cursor pagination: latest page + `before` (load-older) + `after` (incremental tail) with merge-by-id (ties on identical timestamps handled, nothing lost/duplicated); ChatPanel does incremental tail polling + "Load earlier messages" and remounts on booking change; ownership enforced. (Verified: tests/chat.db.test.ts — 60-message latest-page/older/tail with distinct timestamps.) REMAINING: none for pagination.
